...
/// Check if a specific device is currently mounted anywhere
///
/// Parses /proc/mounts to find the device.
pub fn is_device_mounted(device: &Path) -> Result<bool> {
    ...
    for line in mounts.lines() {
        let parts: Vec<&str> = line.split_whitespace().collect();
        if !parts.is_empty() {
            // First field is the device
            if parts[0] == device_str {
                return Ok(true);
            }
        }
    }
...
/// Get the device mounted at a specific mountpoint, if any
pub fn get_mounted_device(mountpoint: &Path) -> Result<Option<String>> {
    let mounts = fs::read_to_string("/proc/mounts").wrap_err("Failed to read /proc/mounts")?;
    ...
    for line in mounts.lines() {
        let parts: Vec<&str> = line.split_whitespace().collect();
        if parts.len() >= 2 && parts[1] == mountpoint_str {
